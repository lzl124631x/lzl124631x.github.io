...
因为我发现, 只要调用了`startRecord`就会触发按钮的`touchcancel`事件. 于是, 之后的`touch`相关事件就不会再被监听了.
...
这些恶心的bug意味着你必须要**限制UI上的操作频率和顺序**(比方说当播放一个录音的时候, 其他播放录音按钮是被禁用的), 也就无法创造一个可以流畅操作(比如随便点击某个录音按钮就可以停掉正在播放的录音而播放新的录音)的用户体验了.
